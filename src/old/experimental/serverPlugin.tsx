...
export interface ServerPlugin<TPluginState = unknown> {
  ...
  /**
   * Get a list of Vite plugin instances that should be used at build-time.
   *
   * This is for advanced use-cases where you might want the ability to perform build-time
   * manipulations of the code. Use-cases include things like:
   * 1. css-in-js libraries that want to perform some AST manipulations at build
   * 2. Code transformations to override `React.lazy` and/or `React.Suspense` on the server
   */
  getBuildPlugins?(
    ctx: Omit<ServerPluginContext, 'state'>
  ): MaybeArray<import('vite').Plugin[]> | undefined;
